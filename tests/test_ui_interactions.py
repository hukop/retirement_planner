...
def test_002_comprehensive_person_profile_sync(dash_duo, p_type):
    ...
    data = {
        "self":   {"name": "Testing Self",   "age": "40", "ret": "60", "ss": "4000", "y_ret": "20", "ss_str": "$4,000/mo"},
        "spouse": {"name": "Testing Spouse", "age": "55", "ret": "65", "ss": "3000", "y_ret": "10", "ss_str": "$3,000/mo"}
    }[p_type]
    ...
    # Current Age
    age_in = dash_duo.find_element(f"#profile-{p_type}-age")
    age_in.send_keys(Keys.CONTROL + "a")
    age_in.send_keys(Keys.BACKSPACE)
    age_in.send_keys(data["age"])
    
    # Retirement Age
    ret_in = dash_duo.find_element(f"#profile-{p_type}-retirement-age")
    ret_in.send_keys(Keys.CONTROL + "a")
    ret_in.send_keys(Keys.BACKSPACE)
    ret_in.send_keys(data["ret"])
    
    # SS Benefit
    ss_in = dash_duo.find_element(f"#profile-{p_type}-ss-benefit")
    ss_in.send_keys(Keys.CONTROL + "a")
    ss_in.send_keys(Keys.BACKSPACE)
    ss_in.send_keys(data["ss"])
    ...
    assert dash_duo.find_element(By.ID, f"profile-{p_type}-age").get_attribute("value") == data["age"]
    assert dash_duo.find_element(By.ID, f"profile-{p_type}-retirement-age").get_attribute("value") == data["ret"]
...
def test_004_load_plan_workflow(dash_duo):
    ...
    test_profile = {
        ...
        "self_person": {"name": "Loaded User", "current_age": 45}
    }
    import json
    import tempfile
